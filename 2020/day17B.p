...
function ActiveNeighbors returns integer (x as integer , y as integer , z as integer , w as integer):
    var int i.
    var int j.
    var int k.
    var int l.
    ...
    do k = -1 to 1:
    do l = -1 to 1:
        if i eq 0 and j eq 0 and k eq 0 and l = 0 then
            next.
        find newcube where newcube.x  eq x + i
                       and newcube.y  eq y + j
                       and newcube.z  eq z + k 
                       and newcube.w  eq w + l no-error. 
...
do cycle = 1 to 6:
        ...
        do l = -1 to 1:
            find first newcube
                 where newcube.x  eq cube.x + i
                   ...
                   and newcube.z  eq cube.z + k 
                   and newcube.w  eq cube.w + l no-error.
            find first bfcube 
                 where bfcube.x  eq cube.x + i
                   and bfcube.y  eq cube.y + j
                   and bfcube.z  eq cube.z + k 
                   and bfcube.w  eq cube.w + l no-error.                
    ...
    for each cube:
        case ActiveNeighbors(cube.x,cube.y,cube.z,cube.w):
            when 2 then 
                next.
            when 3 then do:
