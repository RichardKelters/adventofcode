...
output to 2021/day13answer.txt.
for each dot break by dot.y by dot.x:

    if first-of(dot.y)
    then line = "".
    
    substring(line,dot.x + 1,1) = '#'.
    
    if last-of(y)
    then put unformatted line format 'x(60)' skip.
    
end.
output close.

// show text file for the answer
os-command silent value("start  2021/day13answer.txt").

message  
  "ms:" now - nu skip
  "answer; see text file"
view-as alert-box.
